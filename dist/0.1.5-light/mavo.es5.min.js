...
'use strict';var _toConsumableArray2=require('babel-runtime/helpers/toConsumableArray'),_toConsumableArray3=_interopRequireDefault(_toConsumableArray2);function _interopRequireDefault(a){return a&&a.__esModule?a:{default:a}}(function(a,b){Mavo.attributes.push('mv-plugins');var c=Mavo.Plugins={loaded:{},load:function(){return c.plugins=new Set,b('[mv-plugins]').forEach(function(a){a.getAttribute('mv-plugins').trim().split(/\s+/).forEach(function(a){return c.plugins.add(a)})}),c.plugins.size?a.fetch(c.url+'/plugins.json',{responseType:'json'}).then(function(b){return Mavo.thenAll(b.response.plugin.filter(function(a){return c.plugins.has(a.id)}).map(function(b){var d='mavo-'+b.id+'.js';if(b.repo){var e='https://raw.githubusercontent.com/'+b.repo+'/master/'+d;return c.loaded[b.id]?Promise.resolve():a.fetch(e).then(function(b){a.create('script',{textContent:b.responseText,inside:document.head})})}var e=c.url+'/'+b.id+'/'+d;return a.include(c.loaded[b.id],e)}))}):Promise.resolve()},register:function(b){var d=1<arguments.length&&void 0!==arguments[1]?arguments[1]:{};if(!c.loaded[b]){for(var e in Mavo.hooks.add(d.hooks),d.extend){var f='Mavo'==e?Mavo:Mavo[e];'function'===a.type(f)?a.Class(f,d.extend[e]):a.extend(f,d.extend[e])}var g=[];if(d.ready&&g.push(d.ready),d.dependencies){var h=document.currentScript?document.currentScript.src:location,i=d.dependencies.map(function(a){return Mavo.load(a,h)});g.push.apply(g,(0,_toConsumableArray3.default)(i))}if(g.length){var j;(j=Mavo.dependencies).push.apply(j,g)}c.loaded[b]=d,d.init&&Promise.all(g).then(function(){return d.init()})}},url:'https://plugins.mavo.io'}})(Bliss,Bliss.$);
...
'use strict';var _defineProperty2=require('babel-runtime/helpers/defineProperty'),_defineProperty3=_interopRequireDefault(_defineProperty2);function _interopRequireDefault(a){return a&&a.__esModule?a:{default:a}}(function(a){Mavo.UI.Message=a.Class({constructor:function(b,c,d){var e=this;if(this.mavo=b,this.message=c,this.closed=Mavo.defer(),this.element=a.create((0,_defineProperty3.default)({className:'mv-ui mv-message'+(d.type?' mv-'+d.type:''),innerHTML:this.message,events:{click:function(){return Mavo.scrollIntoViewIfNeeded(e.mavo.element)}}},this.mavo.bar?'after':'start',(this.mavo.bar||this.mavo).element)),d.classes&&this.element.classList.add(d.classes),'error'===d.type?this.element.setAttribute('role','alert'):this.element.setAttribute('aria-live','polite'),d.dismiss=d.dismiss||{},'string'==typeof d.dismiss||Array.isArray(d.dismiss)){var f={};Mavo.toArray(d.dismiss).forEach(function(a){f[a]=!0}),d.dismiss=f}if(d.dismiss.button&&a.create('button',{className:'mv-close mv-ui',textContent:'\xD7',events:{click:function(){return e.close()}},start:this.element}),d.dismiss.timeout){var g,h='number'==typeof d.dismiss.timeout?d.dismiss.timeout:5e3;a.bind(this.element,{mouseenter:function(){return clearTimeout(g)},mouseleave:Mavo.rr(function(){return g=setTimeout(function(){return e.close()},h)})})}d.dismiss.submit&&this.element.addEventListener('submit',function(a){a.preventDefault(),e.close(a.target)})},close:function(b){var c=this;a.transition(this.element,{opacity:0}).then(function(){a.remove(c.element),c.closed.resolve(b)})}})})(Bliss);
